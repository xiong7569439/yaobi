...
// 老记录超时标 stale 的阈值 (OKX 查不到价的 symbol, 如美股代码混入)
...
function recordAlerts(alerts, marketContext = null) {
  ...
  // 构建当前 OKX spot 可用 symbol 集合 (源头过滤美股等非加密资产)
  let validSymbols = null;
  try {
    const tickers = okxFetcher.getSpotTickers();
    if (Array.isArray(tickers) && tickers.length > 0) {
      validSymbols = new Set(tickers.map(t => (t.instId || '').split('-')[0]).filter(Boolean));
    }
  } catch (e) {
    // 获取失败则不过滤, 退到原行为
  }
  ...
  for (const alert of alerts) {
    // 源头拦截: OKX spot 找不到的 symbol 直接不入 tracking (避免僵尸记录)
    if (validSymbols && !validSymbols.has(alert.symbol)) {
      skipNonCrypto++;
      continue;
    }
  ...
  if (skipNonCrypto > 0) {
    console.log(`[Tracker] 源头过滤 ${skipNonCrypto} 条非 OKX spot symbol (美股/其它)`);
  }
}
...
async function checkPending() {
  ...
  // 标记 stale: 超过 48h 仍没拿到任何价格的记录 (OKX 无该 symbol, 如美股代码)
  let staleMarked = 0;
  for (const record of trackingRecords) {
    if (record.status === 'complete' || record.status === 'stale') continue;
    const age = now - record.alertTime;
    if (age > STALE_THRESHOLD && !record.p1h && !record.p4h && !record.p24h) {
      record.status = 'stale';
      record.staleReason = 'OKX spot ticker 未找到该 symbol (可能为非加密资产代码)';
      staleMarked++;
    }
  }
  if (staleMarked > 0) {
    saveTracking();
    console.log(`[Tracker] 标记 ${staleMarked} 条僵尸追踪为 stale`);
  }
}
...
function getTrackingStats() {
  ...
  const stale = trackingRecords.filter(r => r.status === 'stale').length;
  ...
  return { total, pending, partial, complete, stale, avgReturn1h, avgReturn24h };
}
