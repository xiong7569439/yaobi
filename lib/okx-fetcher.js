...
function runOkxCmd(args) {
  try {
    ...
    // OKX 端点返回格式可能是: { details: [...] } 或 [{ details: [...] }]
    if (Array.isArray(raw) && raw.length === 1 && raw[0]?.details) return raw[0].details;
    if (raw && !Array.isArray(raw) && raw.details) return raw.details;
    return raw;
  } catch (err) {
...
async function fullScan() {
  ...
  // 6. 大盘背景 — BTC/ETH 行情
  results.marketContext = extractMarketContext(spotTickers);
...
/**
 * 从 SPOT tickers 中提取 BTC/ETH 大盘背景
 */
